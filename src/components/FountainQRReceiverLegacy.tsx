...
import { FountainDecoder, type FountainMetadata } from '@/utils/fountainCode.legacy'
import { useQRScanner } from '@/hooks/useQRScanner'

interface FountainQRReceiverLegacyProps {
  initialMetadata: {
    name: string
    size: number
    type: string
...
export function FountainQRReceiverLegacy({ initialMetadata }: FountainQRReceiverLegacyProps) {
  ...
  }, [fountainMetadata.type, initialMetadata.checksum, initialMetadata.checksumAlg])
  ...
  const handleScan = useCallback((data: string) => {
    try {
      ...
      // Expect only fountain data chunks
  ...
  return (
    <div className="space-y-4">
      ...
      {!isScanning && !success && (
        <Alert>
          <AlertDescription>
            <p className="font-medium mb-2">📱 Fountain Code Transfer Mode (Simple):</p>
            <ol className="list-decimal list-inside space-y-1 text-sm">
              <li>Click "Start Scanning" to activate camera</li>
              <li>Scan fountain-coded chunks from sender</li>
              <li>You only need ~110% of chunks (can miss some)</li>
              <li>Progress shows decoded blocks, not chunks scanned</li>
              <li>File will download automatically when fully decoded</li>
            </ol>
          </AlertDescription>
        </Alert>
      )}
